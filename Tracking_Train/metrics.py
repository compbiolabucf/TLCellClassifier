...
def cosine_distance(a, b=None):
    ...
    return (
        ...
        tf.matmul(a_normed, tf.transpose(a=b_normed)))
...
def recognition_rate_at_k(probe_x, probe_y, gallery_x, gallery_y, k,
    ...
    num_relevant = tf.minimum(tf.cast(k, tf.float32), tf.reduce_sum(
        input_tensor=label_eq_mat, axis=[1]))
    ...
    true_positives_at_k = tf.reduce_sum(input_tensor=label_eq_mat, axis=[1])
...
def streaming_mean_averge_precision(probe_x, probe_y, gallery_x, gallery_y,
    ...
    num_relevant = tf.reduce_sum(
        input_tensor=good_mask * label_eq_mat, axis=[1], keepdims=True)
    ...
    average_precision = (
        tf.reduce_sum(input_tensor=precision_at_k * relevant_at_k, axis=[1]) /
