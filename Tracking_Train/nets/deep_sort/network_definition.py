...
def create_network(images, num_classes=None, add_logits=True, reuse=None,
    ...
    if create_summaries:
        ...
        tf.compat.v1.summary.image("conv1_1/weights", tf.transpose(
            a=slim.get_variables("conv1_1/weights:0")[0], perm=[3, 0, 1, 2]),
                         max_outputs=128)
    network = slim.conv2d(
        network, 32, [3, 3], stride=1, activation_fn=nonlinearity,
    ...
    if add_logits:
        # with slim.variable_scope("ball", reuse=reuse):
        with tf.compat.v1.variable_scope("ball", reuse=reuse):
            ...
            scale = slim.model_variable(
                "scale", (), tf.float32,
                initializer=tf.compat.v1.constant_initializer(0., tf.float32),
                regularizer=tf.keras.regularizers.l2(0.5 * (1e-1)))
            if create_summaries:
