...
def _parse_filename(filename):
    ...
    person_id, frame_idx = filename_base.split('_')
    return int(person_id), 0, filename_base, ext
...
def read_train_split_to_str(dataset_dir):
    ...
    image_dir = os.path.join(dataset_dir, "Pt210_73_train")
...
def read_train_split_to_image(dataset_dir):
    ...
    images = np.zeros((len(filenames), IMAGE_SHAPE[0], IMAGE_SHAPE[1], IMAGE_SHAPE[2]), np.uint8)
...
def read_test_split_to_str(dataset_dir):
    ...
    image_dir = os.path.join(dataset_dir, "Pt210_73_test")
    for filename in sorted(os.listdir(image_dir)):
        meta_data = _parse_filename(filename)
        if meta_data is None:
            # This is not a valid filename (e.g., Thumbs.db).
    ...
    return gallery_filenames, gallery_ids, None, None, None
...
def read_test_split_to_image(dataset_dir):
    ...
    gallery_images = np.zeros((len(gallery_filenames), IMAGE_SHAPE[0], IMAGE_SHAPE[1], IMAGE_SHAPE[2]), np.uint8)
    for i, filename in enumerate(gallery_filenames):
        gallery_images[i] = cv2.imread(filename, cv2.IMREAD_COLOR)

    query_images = np.zeros((len(query_filenames), IMAGE_SHAPE[0], IMAGE_SHAPE[1], IMAGE_SHAPE[2]), np.uint8)
