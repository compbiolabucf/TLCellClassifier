...
dark_cells = []
for i in range(1000):
    bright_cell = cv2.imread(p0 + f0[i], cv2.IMREAD_GRAYSCALE)#, cv2.IMREAD_GRAYSCALE
    dark_cell = cv2.imread(p1 + f1[i], cv2.IMREAD_GRAYSCALE)#, cv2.IMREAD_GRAYSCALE
    bright_cells.append(bright_cell)
    dark_cells.append(dark_cell)

not_cells = []
for i in range(2000):
    not_cell = cv2.imread(path_not_cell + str(i) + ".tif", cv2.IMREAD_GRAYSCALE)#, cv2.IMREAD_GRAYSCALE
    not_cells.append(not_cell)
...
model = tf.keras.models.Sequential([
  # tf.keras.layers.Conv2D(16, (3,3), activation='relu', input_shape=(10, 10, 3)),
...
ret = det_ml.predict(x_test[300:303])
print(ret)
# print(x_test[0])
